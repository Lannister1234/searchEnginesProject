...
public class QryParser {
  ...
  private static Qry createOperator (String operatorName) {
    ...
    if (operatorNameLowerCase.contains("/")) {
      String[] part = operatorNameLowerCase.split("/");
      operatorNameLowerCase = part[0];
      distance = Integer.parseInt(part[1]);
    }

    //  STUDENT HW1 AND HW2 CODE HdERE
...
//    System.out.println(term);
  ...
  private static Qry parseString (String queryString)
    ...
    while (queryString.length() > 0) {
        ...
        double weight = 0.0;
        String operator = queryTree.getDisplayName().toLowerCase();
        Qry[] qargs = null;
        PopData<String, String> p;

        // handle the weight argument first
        if (operator.equals("#wand") || operator.equals("#wsum")) {
        ...
        if (qargs != null) {
            for (int i = 0; i < qargs.length; i++) {
                ...
                if (operator.equals("#wsum") || operator.equals("#wand")) {
                    ((QrySopW) queryTree).addWeight(weight);
                }
            }
        }
    }
